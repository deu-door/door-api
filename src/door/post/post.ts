...
export async function getPostList(door: Door, courseId: Course['id'], variant: PostVariant): Promise<BasePostHead[]> {
	...
	const postHeads: BasePostHead[] = parseListedTableElement(table)
		.map(row => ({
			...
			author: row['작성자'].text.trim(),
			createdAt: new Date(row['등록일'].text.trim()).toISOString(),
			title: row['제목'].text.trim(),
			views: Number(row['조회'].text.trim()),
			noted: row['읽음'].querySelector('img[alt=확인]')?.tagName.toLowerCase() === 'img',
		}))
...
export async function getPost(door: Door, head: Pick<BasePostHead, 'courseId' | 'variant' | 'id'>): Promise<BasePost> {
	...
	assert(detailTable?.tagName.toLowerCase() === 'table');
	...
	return {
		...
		id,
		courseId,
		title: detail['제목'].text.trim(),
		author: detail['작성자'].text.trim(),
		createdAt: new Date(detail['등록일'].text.trim()).toISOString(),
		views: Number(detail['조회'].text.trim()),
